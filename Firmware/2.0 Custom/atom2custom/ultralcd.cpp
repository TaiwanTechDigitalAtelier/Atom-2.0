...
static void lcd_scale_adjustment();
...
static void menu_action_setting_edit_float32ns(const char* pstr, float* ptr, float minValue, float maxValue);
static void menu_action_setting_edit_float5(const char* pstr, float* ptr, float minValue, float maxValue);
static void menu_action_setting_edit_float51(const char* pstr, float* ptr, float minValue, float maxValue);
static void menu_action_setting_edit_float52(const char* pstr, float* ptr, float minValue, float maxValue);
UNUSED_ static void menu_action_setting_edit_long5(const char* pstr, unsigned long* ptr, unsigned long minValue, unsigned long maxValue);
//static void menu_action_setting_edit_callback_bool(const char* pstr, bool* ptr, menuFunc_t callbackFunc);
UNUSED_ static void menu_action_setting_edit_callback_int3(const char* pstr, int* ptr, int minValue, int maxValue, menuFunc_t callbackFunc);
UNUSED_ static void menu_action_setting_edit_callback_float3(const char* pstr, float* ptr, float minValue, float maxValue, menuFunc_t callbackFunc);
static void menu_action_setting_edit_callback_float32(const char* pstr, float* ptr, float minValue, float maxValue, menuFunc_t callbackFunc);
static void menu_action_setting_edit_callback_float32ns(const char* pstr, float* ptr, float minValue, float maxValue, menuFunc_t callbackFunc);
UNUSED_ static void menu_action_setting_edit_callback_float5(const char* pstr, float* ptr, float minValue, float maxValue, menuFunc_t callbackFunc);
UNUSED_ static void menu_action_setting_edit_callback_float51(const char* pstr, float* ptr, float minValue, float maxValue, menuFunc_t callbackFunc);
...
static void menu_action_setting_edit_ex_bool(const char* pstr, bool* ptr, char *text[], bool invert);
void deploy_z_zero();
...
#define MENU_ITEM_VALUE(type, label, value, sign, interval, args...) do { \
    if (_menuItemNr == _lineNr) { \
        static unsigned long last_update = 0; \
        if (lcdDrawUpdate || millis() > last_update + interval) \
        { \
            const char* _label_pstr = PSTR(label); \
            const char* _value = value; \
            if ((encoderPosition / ENCODER_STEPS_PER_MENU_ITEM) == _menuItemNr) { \
                lcd_implementation_drawmenu_value(_drawLineNr, _label_pstr, _value, sign, '>'); \
            }else{\
                lcd_implementation_drawmenu_value(_drawLineNr, _label_pstr, _value, sign, ' '); \
            }\
            last_update = millis(); \
...
if(false/*lock==1*/){ /*disable lockscreen*/
...
void lcd_preheat1()
{
  setTargetHotend(plaPreheatHotendTemp);
...
void back_to_jct(int extruder)
{
  DIALOG_START(MSG_EXECUTING);
  reset_ext(extruder);
  move_ext(extruder, 5, 300);
  move_ext(extruder, back_to_jct_length, 5000);
  DIALOG_END;
}
...
void press_to_load(int extruder, int val)
{
  DIALOG_START(MSG_EXECUTING);
  reset_ext(extruder);
  do
  {
    move_ext(extruder, val, 1000);
    delay(10);
  } 
  while (READ(BTN_ENC) == 0);
  DIALOG_END;
}
void unload_filament_e0()
{
  unload_filament(0 ^ extruder_inverting);
}
void unload_filament_e1()
{
  unload_filament(1 ^ extruder_inverting);
}
void e0_back_to_jct()
{
  back_to_jct(0 ^ extruder_inverting);
}
void e1_back_to_jct()
{
  back_to_jct(1 ^ extruder_inverting);
}
void load_filament_e0()
{
  load_filament(0 ^ extruder_inverting);
}
void load_filament_e1()
{
  load_filament(1 ^ extruder_inverting);
}
void press_to_load_e0()
{
  press_to_load(0 ^ extruder_inverting, 5);
}
void press_to_load_e1()
{
  press_to_load(1 ^ extruder_inverting, 5);
}
void press_to_unload_e0()
{
  press_to_load(0 ^ extruder_inverting, -5);
}
void press_to_unload_e1()
{
  press_to_load(1 ^ extruder_inverting, -5);
}
void lcd_preheat2()
{
  setTargetHotend(absPreheatHotendTemp);
...
static void lcd_bed_leveling_menu()
{
  ...
  if (!(movesplanned() || IS_SD_PRINTING))
  {
    MENU_ITEM_VALUE(gcode, MSG_PREHEAT_NOZZLE, ftostr3(current_temperature), LCD_STR_DEGREE[0], 400, PSTR("M104 S200"));
    MENU_ITEM(function, MSG_AUTO_CALIBRATE, lcd_g33);
    MENU_ITEM(function, MSG_AUTO_LEVEL, lcd_g29);
    MENU_ITEM(function, MSG_ADJUST_ZOFFSET, lcd_adjuest_zoffet_menu);
    MENU_ITEM(function, MSG_COOLDOWN, lcd_cooldown);
    MENU_ITEM_EDIT_CALLBACK(float32ns, MSG_DELTA_DIAGONAL_ROD, &delta_diagonal_rod, 100, 300, update_delta_diagonal_rod_2);
...
static void lcd_move_x()
{
  if (encoderPosition != 0)
  {
    ...
    encoderPosition = 0;
    calculate_delta(current_position);
    plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS], manual_feedrate[X_AXIS] / 60, CURRENT_EXTRUDER);
    lcdDrawUpdate = 1;
  }
  if (lcdDrawUpdate)
  {
...
static void lcd_move_y()
{
  if (encoderPosition != 0)
  {
    ...
    encoderPosition = 0;
    calculate_delta(current_position);
    plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS], manual_feedrate[Y_AXIS] / 60, CURRENT_EXTRUDER);
    lcdDrawUpdate = 1;
  }
  if (lcdDrawUpdate)
  {
...
static void lcd_move_z()
{
  if (encoderPosition != 0)
  {
    ...
    encoderPosition = 0;
    calculate_delta(current_position);
    plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS], manual_feedrate[Z_AXIS] / 60, CURRENT_EXTRUDER);
    lcdDrawUpdate = 1;
  }
  if (lcdDrawUpdate)
  {
...
static void lcd_move_e()
{
  if (encoderPosition != 0)
  {
    ...
    encoderPosition = 0;
    calculate_delta(current_position);
    plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS], manual_feedrate[E_AXIS] / 60, CURRENT_EXTRUDER);
    lcdDrawUpdate = 1;
  }
  if (lcdDrawUpdate)
  {
...
static void lcd_filaments_menu()
{
  START_MENU();
  MENU_ITEM(back, MSG_MAIN, lcd_main_menu);
  MENU_ITEM(gcode, MSG_DISABLE_EXT_MOTORS, PSTR("M84 E"));
  MENU_ITEM_VALUE(gcode, MSG_PREHEAT_NOZZLE, ftostr3(current_temperature), LCD_STR_DEGREE[0], 400, PSTR("M104 S200"));
  MENU_ITEM(function, MSG_LOAD_FILAMENT_E0, load_filament_e0);
  if (atom_version == 2 || atom_version == 3)
  {
    MENU_ITEM(function, MSG_LOAD_FILAMENT_E1, load_filament_e1);
    MENU_ITEM(function, MSG_E0_BACK_TO_JCT, e0_back_to_jct);
    MENU_ITEM(function, MSG_E1_BACK_TO_JCT, e1_back_to_jct);
    MENU_ITEM(function, MSG_PRESS_TO_LOAD_E0, press_to_load_e0);
    MENU_ITEM(function, MSG_PRESS_TO_LOAD_E1, press_to_load_e1);
    MENU_ITEM(function, MSG_PRESS_TO_UNLOAD_E0, press_to_unload_e0);
    MENU_ITEM(function, MSG_PRESS_TO_UNLOAD_E1, press_to_unload_e1);
  }
...
static void lcd_control_menu()
{
  ...
  MENU_ITEM_EDIT_EX(bool, MSG_EXTRUDER0, &extruder_inverting, extruder_side, false);
  MENU_ITEM_EDIT_EX(bool, MSG_EXTRUDER1, &extruder_inverting, extruder_side, true);
  //MENU_ITEM(submenu, MSG_SCALE_ADJUSTMENT, lcd_scale_adjustment);
...
#endif
  MENU_ITEM_EDIT_EX(int3, MSG_ATOM_VERSION, &atom_version, 0, 3, atom_version_name);
...
static void lcd_scale_adjustment()
{
  START_MENU();
  MENU_ITEM(back, MSG_CONTROL, lcd_control_menu);
  MENU_ITEM_EDIT(float32, MSG_INPUT_SIZE, &input_size, 10, 90);
  MENU_ITEM_EDIT(float32, MSG_OUTPUT_SIZE, &output_size, 10, 90);
  MENU_ITEM(function, MSG_UPDATE_SCALE, update_delta_diagonal_rod_2);
  END_MENU();
}
...
menu_edit_type(float, float32ns, ftostr32ns, 100)
...
void lcd_update()
{
  ...
  if ((IS_SD_INSERTED != lcd_oldcardstatus))
  {
    ...
    {
      card.initsd();
      LCD_MESSAGE_CPGM(MSG_SD_INSERTED);
      SdfileCount = card.getnrfilenames();
    }
    else
    {
      card.release();
      LCD_MESSAGE_CPGM(MSG_SD_REMOVED);
    }
  }
#endif//CARDINSERTED
...
//  convert float to string with 123.45 or -12.34 format
